...
export const NewPostCombined = (props) => {
  ...
  const confirmPost = (event) => {
    ...
    props.createSite({
      projectId: props.projects.active.id,
      title: titleField,
      location: location,
      image: image
    })
    ...
    <div className="newPostContainer centerAlignWithPaddingLean">
      ...
      <div>
        <div className="newPostTabSwitchContainer">
            ...
            {props.settings.strings["choose_location_first"]}
          </p>
        </div>
        <form className="postForm" onSubmit={confirmPost}>
          <div className="postFormButtonContainer">
            <button className="rippleButton negativeButton fillButton" onClick={cancelClick}>
              {props.settings.strings["choose_on_map"]}
            </button>
          </div>
        </form>
	    </div>
      :
        ...
        <form className="postForm" onSubmit={confirmPost}>
          <div className="inputContainer">
	          <p className="normalText textleft">
              {props.settings.strings["give_name_for_location"]}
            </p>
            <input
              name="title"
              id="titleField"
              className="input"
              placeholder={props.settings.strings["name"]}
              maxLength="100"
              autoComplete="off"
              onChange={TitleFieldChangeHandler}
              value={titleField}
            />
          ...
          <div className="postFormButtonContainer">
            <button className="rippleButton positiveButton fillButton">
              {props.settings.strings["submit"]}
            </button>
            <button className="rippleButton negativeButton fillButton" onClick={cancelClick}>
